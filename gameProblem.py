...
class GameProblem(SearchProblem):
   ...
   # --------------- Common functions to a SearchProblem -----------------
    ...
    def actions(self, state):
        ...
        '''
        acciones = []
        east_state = (state[0]+1, state[1])
        south_state = (state[0], state[1]+1)
        north_state = (state[0], state[1]-1)
        west_state = (state[0]-1, state[1])


        if self.getAttribute((east_state),'blocked') != True:
            acciones.append('East')
        
        if self.getAttribute((south_state),'blocked') != True:
            acciones.append('South')

        if self.getAttribute((north_state),'blocked') != True:
            acciones.append('North')

        if self.getAttribute((west_state),'blocked') != True:
            acciones.append('West')

        print(state, ": ", acciones)
        print(self.CONFIG['map_size'])
        return acciones
